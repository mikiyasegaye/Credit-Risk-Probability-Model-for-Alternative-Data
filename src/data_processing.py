...
class DateFeatureExtractor(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        X['Is_NightTime'] = ((X['Transaction_Hour'] >= 22) | (X['Transaction_Hour'] <= 5)).astype(int)
...
class CustomerAggregator(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        aggs['Total_Transaction_Amount'] = X.groupby(self.customer_id)[self.amount_column].sum()
        aggs['Avg_Transaction_Amount'] = X.groupby(self.customer_id)[self.amount_column].mean()
        aggs['Transaction_Count'] = X.groupby(self.customer_id)[self.amount_column].count()
        aggs['Std_Transaction_Amount'] = X.groupby(self.customer_id)[self.amount_column].std()
        aggs['Min_Transaction_Amount'] = X.groupby(self.customer_id)[self.amount_column].min()
        aggs['Max_Transaction_Amount'] = X.groupby(self.customer_id)[self.amount_column].max()
        
        # Value-based aggregations
        aggs['Total_Value'] = X.groupby(self.customer_id)[self.value_column].sum()
        aggs['Avg_Value'] = X.groupby(self.customer_id)[self.value_column].mean()
        
        # Product category diversity
        aggs['Unique_Product_Categories'] = X.groupby(self.customer_id)['ProductCategory'].nunique()
        
        # Channel diversity
        aggs['Unique_Channels'] = X.groupby(self.customer_id)['ChannelId'].nunique()
        
        # Provider diversity
        aggs['Unique_Providers'] = X.groupby(self.customer_id)['ProviderId'].nunique()
        
        # Currency diversity
        aggs['Unique_Currencies'] = X.groupby(self.customer_id)['CurrencyCode'].nunique()
        
        # Subscription count
        aggs['Subscription_Count'] = X.groupby(self.customer_id)['SubscriptionId'].nunique()
...
def main():
    ...
    var_defs = pd.read_csv(os.path.join(raw_dir, 'Xente_Variable_Definitions.csv'))
