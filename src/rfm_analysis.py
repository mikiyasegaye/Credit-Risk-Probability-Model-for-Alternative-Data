...
class RFMAnalyzer:
    ...
    def preprocess_rfm(self, rfm_data):
        ...
        for i, feature in enumerate(["Recency", "Frequency_Log", "Monetary_Log"]):
            ...
            scaled_features[:, i] = (
                self.scalers[feature].fit_transform(feature_values).ravel()
            )
    ...
    def identify_high_risk_cluster(self, rfm_data, cluster_labels):
        ...
        normalized_profiles = self.cluster_profiles.apply(
...
def create_target_variable(data, output_dir=None):
    ...
    if output_dir:
        ...
        rfm_analyzer.cluster_profiles.to_csv(f"{output_dir}/cluster_profiles.csv")

        # Save risk labels with index
        risk_labels.to_csv(f"{output_dir}/risk_labels.csv")
        ...
        try:
            ...
            processed_features = processed_features.loc[
                :, ~processed_features.columns.str.contains("^Unnamed")
            ]

            # Remove any CustomerId columns
            processed_features = processed_features.loc[
                :, ~processed_features.columns.str.contains("CustomerId")
            ]
            ...
            transaction_risks["is_high_risk"] = (
                transaction_risks["CustomerId"]
                .map(risk_labels["is_high_risk"])
                .fillna(0)
                .astype(int)
            )
            ...
            numeric_features = [
                col for col in numeric_features if col != "is_high_risk"
            ]
            ...
            print(
                ...
                risk_labels["is_high_risk"].value_counts(normalize=True),
            )
